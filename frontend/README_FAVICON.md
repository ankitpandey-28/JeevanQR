...
The FastAPI backend serves favicon files from the frontend directory automatically via static mounts, so no extra server code is required.
...
## Testing
1. Start server: `python -m uvicorn app.main:app --reload --host 0.0.0.0 --port 3000`
